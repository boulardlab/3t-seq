localrules: edit_condition_file
    ...
    run:
        ...
        if 'filename' in sample_sheet.columns:
            ...
            if all(sample_sheet.filename_1.apply(lambda x: x.endswith('_sequence'))):
                sample_sheet['tmp'] = sample_sheet.filename_1.apply(lambda x: x.split('_sequence')[0])
        ...
        joined["condition"] = joined.apply(lambda row: "control" if row["genotype"] == "WT" else "treatment",axis=1)
