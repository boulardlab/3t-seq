...
- **Linting (Spell-checking)**:
  ...
  ```bash
  pixi run lint --config remote-references --profile laptop
  ```
...
- **Dry-run (Rehearsal)**:
  ...
  ```bash
  pixi run dry-run --config remote-references --profile laptop
  ```
...
- **Execution (Run)**:
  ...
  ```bash
  pixi run test --config remote-references --profile laptop
  ```
...
```bash
pixi run test-report --config remote-references --profile laptop
```
